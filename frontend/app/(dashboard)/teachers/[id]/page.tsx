...
export default function TeacherProfilePage() {
  ...
  if (!teacher) return <div className="text-center py-16 text-gray-400">Teacher not found</div>
    ...
    <div>
      ...
      <Card className="mb-6 flex items-center gap-6">
        ...
        <div className="text-right">
          <p className="text-xs font-mono text-gray-400 mb-1">Workload</p>
          <div className="w-40"><WorkloadBar value={teacher.workloadPct} /></div>
          <p className="text-xs text-gray-400 font-mono mt-1">{teacher.dutyCount} / {teacher.maxDuties} duties</p>
        </div>
      </Card>
            ...
            <div className="grid grid-cols-4 gap-4 text-center">
              {[
                ...
                <div key={s.label} className="bg-gray-50 rounded-lg p-4">
                  ...
                  <p className="text-xs text-gray-400 font-mono mt-1">{s.label}</p>
                </div>
              ))}
            </div>
          </Card>
        ...
        <Card padding="none">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              ...
              {!(teacher as { duties?: unknown[] }).duties?.length && <tr><td colSpan={6} className="py-8 text-center text-xs text-gray-400">No duties assigned</td></tr>}
            </tbody>
          </table>
        </Card>
      )}
      ...
      {activeTab === 'History' && (
        <Card>
          ...
          {((teacher as { absences?: Record<string, unknown>[] }).absences ?? []).length === 0 ? (
            <p className="text-xs text-gray-400 text-center py-8">No absences recorded</p>
          ) : (
            <table className="w-full text-sm">
              <thead><tr className="bg-gray-50"><th className="px-4 py-2 text-left text-xs font-mono text-gray-400">Date</th><th className="px-4 py-2 text-left text-xs font-mono text-gray-400">Reason</th></tr></thead>
              <tbody>
