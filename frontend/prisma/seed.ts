...
function bcryptHash(password: string): string {
  // Use a simple deterministic hash for seeding (bcrypt not available in ts-node easily)
  // The backend uses passlib — we need to store a bcrypt hash
  // We'll use a pre-computed hash for "Admin@123"
...
async function main() {
  ...
  for (const t of teacherData) {
    ...
    await prisma.user.create({
      data: { id: userId, email: t.email, password: bcryptHash('Teacher@123'), name: t.name, role: 'TEACHER' },
    })
  ...
  const lessonsData = [
    ...
    { tId: sarahId,  subject: 'Calculus',   class_: 'G12A', room: 'A101', day: 'FRI', s: '09:00', e: '09:45' }, // CONFLICT with exam duty
    ...
    { tId: priyaId,  subject: 'Robotics',   class_: 'G11B', room: 'Lab-C2', day: 'FRI', s: '09:15', e: '10:00' }, // Conflict with duty below
  ...
  // 20 duties
  const conflictDutyId = cuid() // Sarah's exam duty on FRI 09:00–12:00 — conflicts with her lesson
  const priyaConflictDutyId = cuid() // Priya conflict duty
  const jamesLunchDutyId = cuid() // James's lunch duty — needs substitution
